...
public class UserServiceImpl implements UserService {
    ...
    @Transactional(readOnly = true)
    @Override
    public JoinTripRequestData getJoinTripRequestData(String email, UUID ownerId) {
        User user = cacheService.getUserById(ownerId);
        String ownerEmail = userRepository.findEmailById(ownerId);

        return new JoinTripRequestData(user.getId(), user.getName(), user.getSurname(), ownerEmail);
    }
